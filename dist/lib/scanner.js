 ...
 */
function isTestOrExample(lineText, context, filename) {
 ...
 */
function getLocationScore(filename) {
    if (!filename)
        return 0;
 ...
 */
function calculateRiskScore(baseScore, entropy, lineText, context, filename) {
    let score = baseScore;
    // Entropy bonus
    if (entropy > 5.0) {
        score += 3;
 ...
 */
function extractContext(lineText, matchStart, matchEnd) {
    ...
    lines.forEach((lineText, index) => {
        ...
        for (const pattern of PATTERNS) {
            ...
            while ((match = pattern.regex.exec(lineText)) !== null) {
                ...
                const riskScore = calculateRiskScore(pattern.baseScore, entropy, lineText, context, filename);
